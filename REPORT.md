...
I then created a plotting script visualizing the difference between an original replay and another replay. I noticed that there is a difference between when the robot inputs occurred indicating some sort of timing mismatch.
...
Although the simulation had consistent movements, I had varying results every time the replay ran.
## 4. Part 2: Data Visualization and Trajectory Comparison

I have attached below the initial visualization of the original PSM kinematics of both the left arm and right arm
| Left Arm | Right Arm |
| :---: | :---: |
...
This desynchronization is due to how packets are sent through the replay script. After further analysis of replay.py and specifically the variables `time_delta (line 44)` and `sleep_time (line 45)`, we can see that sleep_time varies wildly due to the script being relative to the previous packet. After printing the values sleep_time was, I noticed that it can vary significantly with the sleep_time ranging from numbers like 1.38e-05 to 0.01 and also sometimes 0. This varying sleep_time leads to drift over time as the differences compound throughout the replay. 
...
- Added variables for output directory and how many n frames for easy changing (Lines 56-60)
...
In part 2, I would like to work on fixing this time discrepancy in the future. 

One main limitation is in part 3 with how the rgb, depth, and object segmentation images are saved. In the future, I would hope to fix this issue and look into what is causign the problem more. 
## 12. GenAI Use Disclosure
Generative AI, specifically Google Gemini, was used to help setup the environment as I was initially running into several errors setting up the simulator. 
